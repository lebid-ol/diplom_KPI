...
def register_view(request):
    ...
    if form.is_valid():
        ...
        new_user.save()
        messages.success(request, 'Користувач добавлений у систему.')
...
def delete_view(request):
    if request.user.is_authenticated:
        ...
        if request.method == 'POST':
            ...
            qs.delete()
            messages.error(request, 'Користувач видалений :(')
    return redirect('home')
...
def contact(request):
    if request.method == 'POST':
        ...
        if contact_form.is_valid():
            ...
            messages.success(request, 'Повідомлення відправлено адміністрації.')
            return redirect('accounts:update')
        else:
            return redirect('accounts:update')
    else:
